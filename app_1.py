...
user_books_path = "liked_books.csv"
book_id_map_path = "book_id_map.csv"
interactions_path = "goodreads_interactions.csv"
books_titles_path = "books_titles.json"

# Button to get recommendations
if st.button("Get Collaborative Recommendations"):
    top_recs = get_collaborative_recommendations(
        user_books_path,
        book_id_map_path,
        interactions_path,
        books_titles_path
    ...
    if not top_recs.empty:
        st.subheader("Recommended Books Based on Your Preferences")
        for _, row in top_recs.iterrows():
            st.image(row['cover_image'], width=150)
            st.markdown(f"**{row['title']}**")
            st.text(f"Average Rating: {row['mean']:.2f}")
            st.markdown(f"[More details]({row['url']})", unsafe_allow_html=True)
            st.write("---")
    else:
